...
export default function SearchPlaces({
  ...
  SearchPlaces.propTypes = {
    plannerDetails: PropTypes.object,
    handleSelect: PropTypes.func,
    handleSearch: PropTypes.func,
    validate: PropTypes.object,
    plannerId: PropTypes.object,
  };
  ...
  const ListofDates = () =>
    plannerDetails?.map((item, idx) => {
      return (
        <li key={idx} onClick={handleClicker}>
          <a>{dayjs(item.date).format("DD-MM-YYYY")} </a>
        </li>
      );
    });
  ...
  const handleClick = (e) => {
    ...
    setIsOpen(true);
    const findIndex = resultState.findIndex(
    ...
    return (
      <>
        ...
        <div className="carousel carousel-center rounded-box">
          {selectedResult?.photos?.map((item, idx) => (
  ...
  const AddingOption = () => {
    return (
      <details className="dropdown" ref={dropdown} id="dropdown">
    ...
    resultState?.map((item, idx) => {
      ...
      return (
        ...
        >
          ...
          >
            <div
              className="collapse-title text-m font-small"
            ...
            <div>
              {Object.keys(detailResult).length !== 0 &&
                detailResult[item.place_id]?.formatted_address}
            </div>
            {item.rating && (
  ...
  const handleAddToItinerary = async (e) => {
    ...
    if (selectedDay !== "0") {
      ...
      const data = {
        planner_items_id: selectedDay,
        name: selectedResult.name,
        locations: [geometry.location.lng, geometry.location.lat],
        plannerId: plannerId,
  ...
  const ListOptions = () =>
    ...
    plannerDetails?.map((item, idx) => {
      ...
      return (
        <option key={idx} value={parseInt(item.planner_items_id)}>
